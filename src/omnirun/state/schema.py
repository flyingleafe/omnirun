...
# Phase-3 budget ledger (DESIGN §7). Append-only log of committed/spent cost per
# job, keyed by rolling window ("day"/"week"). ``at`` is ISO-8601. The
# (window, at) index serves the window-scoped range scan in ``load_ledger``.
# There is NO ``placements`` table: a job's ``Placement`` lives on the jobs
# ``data`` blob, and the indexed jobs ``backend``+``state`` columns drive the
# per-provider active count.
ledger = Table(
    "ledger",
    metadata,
    Column("id", Integer, primary_key=True, autoincrement=True),
